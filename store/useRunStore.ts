...
export type RunState = {
  runId?: string;
  status: PhaseStatus;
  currentPhase: 1 | 2 | 3 | 4 | 0;
  phases: Record<1 | 2 | 3 | 4, PhaseStatus>;
  theater: Record<1 | 2 | 3 | 4, TheaterLog[]>;
  ...
  setPhaseStatus: (p: 1 | 2 | 3 | 4, s: PhaseStatus) => void;
...
export const useRunStore = create<RunState>()((set, get) => ({
  ...
  status: "idle",
  currentPhase: 0,
  phases: { 1: "idle", 2: "idle", 3: "idle", 4: "idle" },
  theater: { 1: [], 2: [], 3: [], 4: [] },
  results: {},
  ...
  pushLog: (log) =>
    set((st) => {
      const existing = st.theater[log.phase as 1|2|3|4] || [];
      const max = 50;
      const next = [...existing, log].slice(-max);
      return { theater: { ...st.theater, [log.phase as 1|2|3|4]: next } };
    }),
  setResult: (result) =>
